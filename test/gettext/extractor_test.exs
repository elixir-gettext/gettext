defmodule Gettext.ExtractorTest do
  ...
  describe "merge_template/2" do
    test "non-autogenerated translations are kept" do
      ...
      t1 = %Message.Singular{msgid: ["foo"], msgstr: ["bar"]}
      t2 = %Message.Singular{msgid: ["baz"], msgstr: ["bong"]}
      t3 = %Message.Singular{msgid: ["a", "b"], msgstr: ["c", "d"]}
      old = %Messages{messages: [t1]}
      new = %Messages{messages: [t2, t3]}

      assert Extractor.merge_template(old, new, []) == %Messages{messages: [t1, t2, t3]}
    end

    test "allowed translations are kept" do
      t1 = %Message.Singular{
        msgid: ["foo"],
        msgstr: ["bar"],
        references: [{"foo.ex", 1}],
    ...
    test "obsolete autogenerated translations are discarded" do
      ...
      t2 = %Message.Singular{msgid: ["baz"], msgstr: ["bong"]}
      old = %Messages{messages: [t1]}
      new = %Messages{messages: [t2]}

      assert Extractor.merge_template(old, new, []) == %Messages{messages: [t2]}
    end
    ...
    test "order is kept as much as possible" do
      ...
      foo_translation = %Message.Singular{msgid: ["foo"], references: [{"foo.ex", 1}]}
      ...
      po2 = %Messages{
        messages: [
          %Message.Singular{msgid: ["new translation"]},
          foo_translation,
          %Message.Singular{msgid: [msgid], references: [{"live_streaming.ex", 40}]}
        ]
      }

      %Messages{messages: [t1, ^foo_translation, t2]} = Extractor.merge_template(po1, po2, [])
    ...
    test "translations can be ordered alphabetically through the :sort_by_msgid option" do
      ...
      bar_translation = %Message.Singular{msgid: ["bar"], references: [{"bar.ex", 1}]}

      baz_translation = %Message.Plural{
        msgid: ["baz"],
        msgid_plural: ["bazs"],
        references: [{"baz.ex", 1}]
      }

      qux_translation = %Message.Singular{msgid: ["qux"], references: [{"bar.ex", 1}]}
      ...
      %Messages{messages: translations} = Extractor.merge_template(po1, po2, sort_by_msgid: true)
