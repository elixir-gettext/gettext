defmodule Gettext.PO do
  ...
  always be a POT file , but `old` can be a POT file as well as a PO file (see
  the `:pot_onto_po` option). `new` has precedence over `old` where it makes sense.
  ...
  ## Options
    ...
    * `:pot_onto_po` - if `true`, this function behaves slightly differently as
      it assumes `old` is a PO file and `new` is a POT file. The only thing that
      changes is that the `msgstr` of the translations that appear both in `old`
      and `new` will be the one in `old` instead of the one in `new` (which is
      the behaviour when this option is `false`). Defaults to `false`.

  """
  @spec merge(t, t, Keyword.t) :: t
  def merge(old, new, opts \\ []) do
    keep_manual? = Keyword.get(opts, :keep_manual_translations, true)
    pot_onto_po? = Keyword.get(opts, :pot_onto_po, false)
    ...
    old_and_merged = Enum.flat_map old.translations, fn(t) ->
      cond do
        ...
        same = find_translation(new.translations, t) ->
          [Translations.merge(t, same, pot_onto_po: pot_onto_po?)]
