defmodule Gettext.PO do
  ...
  This function is used to merge two `Gettext.PO` structs into one. `new` has
  precedence over `old`, meaning that:

    * all headers from `old` are discarded (only the ones from `new` are kept)
    * autogenerated translations in `old` that are not in `new` are discarded

  """
  @spec merge(t, t) :: t
  def merge(old, new) do
    {common_old, unique_old} = Enum.partition old.translations, fn(t) ->
      Enum.find new.translations, &Translations.same?(t, &1)
    end

    {common_new, unique_new} = Enum.partition new.translations, fn(t) ->
      Enum.find old.translations, &Translations.same?(t, &1)
    end

    translations =
      discard_autogenerated(unique_old) ++
      merge_translations(common_old, common_new) ++
      unique_new

    %PO{translations: translations, headers: merge_headers(old.headers, new.headers)}
  end

  defp merge_translations(old, new) do
    for {old_t, new_t} <- Enum.zip(Enum.sort(old), Enum.sort(new)) do
      Translations.merge(old_t, new_t)
    end
  end

  defp merge_headers(_old, new) do
    new
  end

  defp discard_autogenerated(translations) do
    Enum.reject(translations, &Translations.autogenerated?/1)
  end
end
